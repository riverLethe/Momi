...
const EnhancedDonutChart: React.FC<EnhancedDonutChartProps> = ({ 
  ...
  const displayedCategories = showAllCategories ? data.sort((a, b) => b.value - a.value) : data.sort((a, b) => b.value - a.value).slice(0, 5);
    ...
    <YStack alignItems="center" justifyContent="center" paddingVertical="$3">
      <View style={{ width: size, height: size, position: 'relative' }}>
        <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
          <SvgCircle cx={center} cy={center} r={radius} fill="transparent" stroke="#f1f5f9" strokeWidth={strokeWidth} />
          {segments.map((segment, i) => <Path key={i} d={segment.path} fill={segment.color} />)}
          <SvgCircle cx={center} cy={center} r={radius - strokeWidth} fill="white" />
        </Svg>
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, justifyContent: 'center', alignItems: 'center' }}>
          <Text fontWeight="$8" fontSize="$6">¥{totalValue.toFixed(0)}</Text>
          <Text color="$gray10" fontSize="$2.5" marginTop="$1">{t("Total Expenses")}</Text>
        </View>
      </View>
      ...
      <Card width="100%" padding="$3.5" borderRadius="$4" marginTop="$3" backgroundColor="white" elevate>
            ...
            <YStack key={item.label} marginBottom="$2.5">
              ...
              {hasChange && (
                ...
                <XStack alignItems="baseline" space="$1">
                  <Text fontWeight="$7" fontSize="$3">¥{item.value.toFixed(0)}</Text>
                  <Text color="$gray10" fontSize="$2">({((item.value / totalValue) * 100).toFixed(1)}%)</Text>
                </XStack>
              </XStack>
            </YStack>
          );
        ...
        {hasMoreCategories && (
          <Button marginTop="$2" size="$3" variant="outlined" onPress={() => setShowAllCategories(!showAllCategories)}>
